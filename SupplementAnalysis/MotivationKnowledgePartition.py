...
def PriorKnowledgePartition(course_path, personality_path, survey_path):
    ...
    for row in reader:
        ...
        if learner in learner_personality_map.keys():
            ...
            prior_learner_map[learner] = prior_score
    ...
    print "# survey learners is:\t" + str(len(prior_learner_map))
    
    update_learner_personality_map = {}    
    for learner in prior_learner_map.keys():
        update_learner_personality_map[learner] = learner_personality_map[learner]
        update_learner_personality_map[learner]["prior_knowledge"] = prior_learner_map[learner]
    ...
    for i in range(bins):
        ...
        for j in [0, 1]:
            ...
            output_path = course_path + "supplement/partition_" + str(i) + "_" + str(j)
            output_file = open(output_path, "w")
            writer = csv.writer(output_file)
            ...
            id_output_path = course_path + "supplement/partition_id_" + str(i) + "_" + str(j)
            ...
            for learner in learner_personality_map.keys():
                ...
                motivation = learner_personality_map[learner]["motivation"]
            
                start = bin_array[i][0]
                end = bin_array[i][1]
            
                if prior_knowledge >= start and prior_knowledge < end and motivation == j:
